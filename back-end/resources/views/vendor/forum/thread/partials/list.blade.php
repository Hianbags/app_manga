<div class="list-group-item {{ $thread->pinned ? 'pinned' : '' }} {{ $thread->locked ? 'locked' : '' }} {{ $thread->trashed() ? 'deleted' : '' }}" :class="{ 'border-primary': selectedThreads.includes({{ $thread->id }}) }">
    <div class="row align-items-center text-center">
        ...
        <div class="col-sm text-md-end">
            ...
            <!-- Chỉ hiển thị nút duyệt nếu chủ đề chưa được duyệt -->
            @if (!$thread->approve == 1)
                <form action="{{ Forum::route('thread.approve', $thread) }}" method="POST" style="display:inline;">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-success">{{ trans('Duyệt') }}</button>
                </form>
            @endif
        </div>
